...
#[derive(Debug, Clone, PartialEq, Eq, sqlx::FromRow)]
struct ParcelCommandRouteTarget {
    id: i64,
    desk_id: i64,
    parcel_id: String,
    slug: String,
...
impl PgStorage {
    /// Creates a mailing list for an owned built parcel parcel.
    pub async fn create_parcel_mailing_list(
        ...
        slug: &str,
        title: &str,
    ) -> Result<StoredParcelMailingList, StorageError> {
        validate_slug(slug)?;
        validate_title(title)?;
        let parcel = self.owned_built_parcel(parcel_id, owner_player_id).await?;
        if self
            .parcel_mailing_list_by_parcel_slug(parcel.parcel_id.as_str(), slug)
            .await?
            .is_some()
        {
        ...
        let list_count = self.parcel_mailing_list_count(&parcel.parcel_id).await?;
        ...
        let row = sqlx::query_as::<_, StoredParcelMailingList>(
            r#"
            insert into parcel_mailing_lists (parcel_id, owner_player_id, slug, title)
            values ($1, $2, $3, $4)
    ...
    /// Lists mailing lists for an owned parcel parcel.
    pub async fn parcel_mailing_lists(
        &self,
        parcel_id: &str,
        owner_player_id: &str,
    ) -> Result<Vec<StoredParcelMailingList>, StorageError> {
        let parcel = self.owned_built_parcel(parcel_id, owner_player_id).await?;
        self.parcel_mailing_lists_for_parcel(&parcel.parcel_id)
            .await
    }

    /// Lists mailing lists for a parcel without applying owner authorization.
    pub async fn parcel_mailing_lists_for_parcel(
        &self,
        parcel_id: &str,
    ) -> Result<Vec<StoredParcelMailingList>, StorageError> {
        let parcel_id = canonical_parcel_id(parcel_id);
        let rows = sqlx::query_as::<_, StoredParcelMailingList>(
            r#"
            ...
            from parcel_mailing_lists l
            left join lateral (
                select count(*) as count
                from parcel_mailing_list_subscriptions s
    ...
    pub async fn parcel_mailing_list_subscribers(
        ...
        owner_player_id: &str,
        limit: i64,
    ) -> Result<ParcelMailingListSubscriberPage<StoredParcelMailingListSubscriber>, StorageError>
    {
        let list = self
            .owned_parcel_mailing_list(parcel_id, slug, owner_player_id)
            .await?;
        let subscribers = sqlx::query_as::<_, StoredParcelMailingListSubscriber>(
            r#"
            select subscriber_user, subscriber_player_id,
                   to_char(updated_at, 'YYYY-MM-DD HH24:MI:SS TZ') as updated_at
            from parcel_mailing_list_subscriptions
            where list_id = $1
              and status = $3
        ...
        let row = sqlx::query_as::<_, StoredParcelMailingList>(
            ...
            returning id, parcel_id, owner_player_id, slug, title, status,
                      (
                          select count(*)::bigint
                          from parcel_mailing_list_subscriptions s
                          where s.list_id = parcel_mailing_lists.id
                            and s.status = $3
                      ) as subscriber_count,
                      to_char(created_at, 'YYYY-MM-DD HH24:MI:SS TZ') as created_at
            "#,
    ...
    /// Resolves a built parcel target and mailing-list slug without changing membership.
    pub async fn parcel_mailing_list(
        &self,
        target: &str,
        slug: &str,
    ) -> Result<StoredParcelMailingList, StorageError> {
        validate_slug(slug)?;
        self.resolve_parcel_mailing_list(target, slug).await
    }
    ...
    ) -> Result<StoredParcelMailingListSubscription, StorageError> {
        validate_slug(slug)?;
        let list = self.resolve_parcel_mailing_list(target, slug).await?;
    ...
    pub async fn parcel_mailing_list_subscriptions(
        &self,
        subscriber_player_id: &str,
    ) -> Result<Vec<StoredParcelMailingListSubscription>, StorageError> {
        let rows = sqlx::query_as::<_, StoredParcelMailingListSubscription>(
            r#"
            select l.parcel_id, p.title as parcel_title, l.slug, l.title as list_title,
                   s.status,
                   to_char(s.updated_at, 'YYYY-MM-DD HH24:MI:SS TZ') as updated_at
            from parcel_mailing_list_subscriptions s
            join parcel_mailing_lists l on l.id = s.list_id
    ...
    /// Sends one mailing-list post to all active parcel-chat members.
    pub async fn send_parcel_mailing_list_post(
        ...
        subject: &str,
        body: &str,
    ) -> Result<ParcelMailingListSend<StoredParcelMailingListPost, StoredInboxItem>, StorageError>
    {
        validate_slug(slug)?;
        ...
        let sender_has_subscription = self
            ...
            .await?;
        if !sender_can_post_to_parcel_chat(
        ...
        let post = self
            .insert_parcel_mailing_list_post_for_recipients(MailingListPostInsert {
                list_id: list.id,
                parcel_id: &list.parcel_id,
                slug: &list.slug,
        ...
        let deliveries = self.deliver_parcel_mailing_list_post(post.id).await?;
        Ok(ParcelMailingListSend { post, deliveries })
    }

    /// Creates a parcel-local work desk for an owned built parcel parcel.
    pub async fn create_parcel_work_desk(
        ...
        slug: &str,
        title: &str,
    ) -> Result<StoredParcelWorkDesk, StorageError> {
        validate_work_slug(slug)?;
        validate_work_title(title)?;
        let parcel = self.owned_built_parcel(parcel_id, owner_player_id).await?;
        if self
            .parcel_work_desk_by_parcel_slug(&parcel.parcel_id, slug)
            .await?
            .is_some()
        {
            return Err(StorageError::ParcelWorkDeskAlreadyExists {
                parcel_id: parcel.parcel_id,
                slug: slug.to_owned(),
            });
        }
        let desk_count = self.parcel_work_desk_count(&parcel.parcel_id).await?;
        if desk_count >= PARCEL_WORK_DESKS_PER_PARCEL_MAX {
            return Err(StorageError::InvalidParcelWork(format!(
                "work-desk limit reached for parcel {}; maximum is {}",
                parcel.parcel_id, PARCEL_WORK_DESKS_PER_PARCEL_MAX
            )));
        }
        let row = sqlx::query_as::<_, StoredParcelWorkDesk>(
            r#"
            insert into parcel_work_desks (parcel_id, owner_player_id, slug, title)
            values ($1, $2, $3, $4)
            returning id, parcel_id, owner_player_id, slug, title, status,
    ...
    /// Lists parcel-local work desks for an owned parcel parcel.
    pub async fn parcel_work_desks(
        &self,
        parcel_id: &str,
        owner_player_id: &str,
    ...
    pub async fn add_parcel_staff(
        ...
        owner_player_id: &str,
        username: &str,
    ) -> Result<StoredParcelStaff, StorageError> {
        validate_work_slug(slug)?;
        let username = username.trim();
        if username.is_empty() || username.contains(char::is_whitespace) {
            return Err(StorageError::InvalidParcelWork(
        ...
        let desk = self
            ...
            .await?;
        let staff = sqlx::query_as::<_, StoredParcelStaff>(
            r#"
            insert into parcel_work_staff (desk_id, staff_user, status)
            values ($1, $2, $3)
    ...
    pub async fn parcel_staff(
        &self,
        parcel_id: &str,
        slug: &str,
        owner_player_id: &str,
        limit: i64,
    ) -> Result<Vec<StoredParcelStaff>, StorageError> {
        validate_work_slug(slug)?;
        let desk = self
            .owned_parcel_work_desk(parcel_id, slug, owner_player_id)
            .await?;
        let rows = sqlx::query_as::<_, StoredParcelStaff>(
            r#"
            select staff_user, status,
                   to_char(updated_at, 'YYYY-MM-DD HH24:MI:SS TZ') as updated_at
            from parcel_work_staff
            where desk_id = $1
    ...
    pub async fn remove_parcel_staff(
        ...
        .await?
        .ok_or_else(|| StorageError::ParcelWorkerNotAssigned {
            parcel_id: desk.parcel_id.clone(),
            slug: desk.slug.clone(),
        })?;
        sqlx::query(
            r#"
            update parcel_work_shifts
    ...
    ) -> Result<StoredParcelShift, StorageError> {
        validate_work_slug(slug)?;
        let desk = self.parcel_work_desk(parcel_id, slug).await?;
        self.ensure_worker_assigned(&desk, worker_user, worker_player_id)
            .await?;
        if let Some(shift) = self.active_parcel_shift(desk.id, worker_player_id).await? {
            return Ok(shift);
        }
        let shift = sqlx::query_as::<_, StoredParcelShift>(
            r#"
            insert into parcel_work_shifts (desk_id, worker_user, worker_player_id, status)
            values ($1, $2, $3, $4)
            returning id,
                      $5::text as parcel_id,
                      $6::text as slug,
    ...
    pub async fn end_parcel_shift(
        ...
        .fetch_optional(&self.pool)
        .await?
        .ok_or_else(|| StorageError::ParcelShiftNotActive {
    ...
    pub async fn parcel_work_items(
        &self,
        parcel_id: &str,
        worker_user: &str,
        worker_player_id: &str,
        slug: Option<&str>,
        limit: i64,
    ) -> Result<Vec<StoredParcelWorkItem>, StorageError> {
        ...
        if desk_ids.is_empty() {
            ...
            return Err(StorageError::ParcelShiftNotActive {
                parcel_id: canonical_parcel_id(parcel_id).into_owned(),
                slug,
            });
        }
        self.parcel_work_items_for_desks(&desk_ids, worker_player_id, limit)
            .await
    }
    ...
    pub async fn claim_parcel_work(
        ...
        worker_player_id: &str,
        work_id: i64,
    ) -> Result<StoredParcelWorkItem, StorageError> {
        let item = self.parcel_work_item(work_id).await?;
        if item.parcel_id != canonical_parcel_id(parcel_id).as_ref() {
            return Err(StorageError::ParcelWorkItemNotFound(work_id));
        }
        ...
        let claim_query = format!(
            ...
            set status = $3,
                ...
                updated_at = now()
            from parcel_work_desks d, operator_commands cmd
            where item.id = $1
              and item.status = $2
            ...
            returning {}
            "#,
            parcel_work_item_projection()
        );
        let claimed = sqlx::query_as::<_, StoredParcelWorkItem>(&claim_query)
            .bind(work_id)
    ...
    pub async fn finish_parcel_work(
        ...
        let done = sqlx::query_as::<_, StoredParcelWorkItem>(&done_query)
            .bind(work_id)
            .bind(PARCEL_WORK_ITEM_CLAIMED)
    ...
    /// Adds or returns a command route from an owned parcel into one work desk.
    pub async fn add_parcel_command_route(
        ...
        let desk = self
            .owned_parcel_work_desk(parcel_id, slug, owner_player_id)
            .await?;
        let route = sqlx::query_as::<_, StoredParcelCommandRoute>(
            r#"
            insert into parcel_work_routes (
    ...
    /// Lists command routes for an owned parcel parcel.
    pub async fn parcel_command_routes(
        &self,
        parcel_id: &str,
        owner_player_id: &str,
    ) -> Result<Vec<StoredParcelCommandRoute>, StorageError> {
        let parcel = self.owned_built_parcel(parcel_id, owner_player_id).await?;
        let routes = sqlx::query_as::<_, StoredParcelCommandRoute>(
            r#"
            select r.id, r.parcel_id, d.slug, d.title as desk_title,
                   r.command_prefix,
                   to_char(r.created_at, 'YYYY-MM-DD HH24:MI:SS TZ') as created_at
            from parcel_work_routes r
            join parcel_work_desks d on d.id = r.desk_id
            where r.parcel_id = $1
    ...
    pub async fn remove_parcel_command_route(
        ...
        .await?
        .ok_or_else(|| {
            StorageError::InvalidParcelWork(format!(
        ...
        sqlx::query("delete from parcel_work_routes where id = $1")
            .bind(route.id)
            .execute(&self.pool)
            .await?;
        Ok(route)
    ...
    pub async fn dispatch_parcel_command_routes<P>(
        &self,
        parcel: &P,
        command_id: i64,
    ) -> Result<Vec<StoredParcelWorkItem>, StorageError>
        ...
        P: ParcelView,
    {
        ...
        let targets = matching_parcel_command_route_targets(
            self.parcel_command_route_targets(parcel.parcel_id())
                .await?,
            &command.raw_input,
        );
        let mut items = Vec::new();
        for target in targets {
            let item = self.insert_parcel_work_item(&target, command.id).await?;
            items.push(item);
        }
        Ok(items)
    }
    ...
    pub async fn deliver_parcel_mailing_list_post(
        &self,
        post_id: i64,
    ) -> Result<Vec<ParcelMailingListDelivery<StoredInboxItem>>, StorageError> {
        let post = self.parcel_mailing_list_post(post_id).await?;
        let recipients = sqlx::query_as::<_, MailingListDeliveryRecipient>(
            r#"
            select id, recipient_user, recipient_player_id
            from parcel_mailing_list_deliveries
            where post_id = $1
            order by id
            "#,
        )
        ...
        for recipient in recipients {
            ...
            sqlx::query(
                r#"
                update parcel_mailing_list_deliveries
                set inbox_item_id = $2
                where id = $1
                "#,
            )
            ...
            .execute(&self.pool)
            .await?;
            deliveries.push(ParcelMailingListDelivery {
                recipient_player_id: recipient.recipient_player_id,
                inbox_item,
            });
        }
    ...
    async fn owned_parcel_work_desk(
        ...
        validate_work_slug(slug)?;
        let parcel = self.owned_built_parcel(parcel_id, owner_player_id).await?;
        self.parcel_work_desk_by_parcel_slug(&parcel.parcel_id, slug)
            .await?
            .ok_or_else(|| StorageError::ParcelWorkDeskNotFound {
                parcel_id: parcel.parcel_id,
                slug: slug.to_owned(),
            })
    }

    async fn parcel_work_desk(
        &self,
        parcel_id: &str,
        slug: &str,
    ) -> Result<StoredParcelWorkDesk, StorageError> {
        validate_work_slug(slug)?;
        let parcel_id = canonical_parcel_id(parcel_id);
        self.parcel_work_desk_by_parcel_slug(parcel_id.as_ref(), slug)
            .await?
            .ok_or_else(|| StorageError::ParcelWorkDeskNotFound {
    ...
    async fn parcel_work_desk_by_parcel_slug(
        &self,
        parcel_id: &str,
        slug: &str,
    ) -> Result<Option<StoredParcelWorkDesk>, StorageError> {
        let row = sqlx::query_as::<_, StoredParcelWorkDesk>(
            r#"
            ...
            from parcel_work_desks d
            left join lateral (
                select count(*) as count
                from parcel_work_items item
                where item.desk_id = d.id
                  and item.status = $3
            ) queued_items on true
            left join lateral (
                select count(*) as count
                from parcel_work_shifts shift
    ...
    async fn parcel_work_desks_for_parcel(
        &self,
        parcel_id: &str,
    ) -> Result<Vec<StoredParcelWorkDesk>, StorageError> {
        let parcel_id = canonical_parcel_id(parcel_id);
        let rows = sqlx::query_as::<_, StoredParcelWorkDesk>(
            r#"
            select d.id, d.parcel_id, d.owner_player_id, d.slug, d.title, d.status,
                   coalesce(queued_items.count, 0)::bigint as queued_count,
                   coalesce(active_workers.count, 0)::bigint as active_worker_count,
    ...
    async fn parcel_work_desk_count(&self, parcel_id: &str) -> Result<usize, StorageError> {
        let count = sqlx::query_scalar::<_, i64>(
            r#"
            select count(*)::bigint
            from parcel_work_desks
            where parcel_id = $1
            "#,
        )
        .bind(parcel_id)
        ...
        usize::try_from(count).map_err(|_| {
            StorageError::InvalidParcelWork("work-desk count exceeds supported range".to_owned())
        })
    }

    async fn ensure_worker_assigned(
        &self,
        desk: &StoredParcelWorkDesk,
        worker_user: &str,
        worker_player_id: &str,
    ) -> Result<(), StorageError> {
        ...
        if assigned {
            ...
            Err(StorageError::ParcelWorkerNotAssigned {
                parcel_id: desk.parcel_id.clone(),
                slug: desk.slug.clone(),
            })
        }
    ...
    async fn active_parcel_shift(
        &self,
        desk_id: i64,
        worker_player_id: &str,
    ) -> Result<Option<StoredParcelShift>, StorageError> {
        let row = sqlx::query_as::<_, StoredParcelShift>(
            r#"
            ...
            from parcel_work_shifts shift
            join parcel_work_desks d on d.id = shift.desk_id
    ...
    async fn active_worker_desk_ids(
        ...
        let rows = sqlx::query_scalar::<_, i64>(
            ...
            from parcel_work_desks d
            join parcel_work_shifts shift on shift.desk_id = d.id
            where d.parcel_id = $1
    ...
    async fn parcel_work_items_for_desks(
        &self,
        desk_ids: &[i64],
        worker_player_id: &str,
        limit: i64,
        ...
        let query = format!(
            ...
            select {}
            from parcel_work_items item
            join parcel_work_desks d on d.id = item.desk_id
        ...
        let rows = sqlx::query_as::<_, StoredParcelWorkItem>(&query)
            .bind(desk_ids)
            .bind(PARCEL_WORK_ITEM_QUEUED)
            .bind(PARCEL_WORK_ITEM_CLAIMED)
            .bind(worker_player_id)
    ...
    async fn parcel_work_item(&self, work_id: i64) -> Result<StoredParcelWorkItem, StorageError> {
        let query = format!(
            r#"
            select {}
            from parcel_work_items item
        ...
        sqlx::query_as::<_, StoredParcelWorkItem>(&query)
            .bind(work_id)
            .fetch_optional(&self.pool)
            .await?
            .ok_or(StorageError::ParcelWorkItemNotFound(work_id))
    }

    async fn ensure_active_shift_for_work(
        &self,
        item: &StoredParcelWorkItem,
        worker_user: &str,
        worker_player_id: &str,
    ) -> Result<(), StorageError> {
        let desk = self.parcel_work_desk(&item.parcel_id, &item.slug).await?;
        self.ensure_worker_assigned(&desk, worker_user, worker_player_id)
            .await?;
        if self
            .active_parcel_shift(desk.id, worker_player_id)
            .await?
            .is_some()
        {
            Ok(())
        } else {
            Err(StorageError::ParcelShiftNotActive {
    ...
    async fn insert_parcel_work_item(
        &self,
        target: &ParcelCommandRouteTarget,
        command_id: i64,
    ) -> Result<StoredParcelWorkItem, StorageError> {
        let query = format!(
            r#"
            insert into parcel_work_items as item (
            ...
            returning {}
            "#,
            parcel_work_item_projection_with_subqueries()
        );
        sqlx::query_as::<_, StoredParcelWorkItem>(&query)
    ...
    async fn owned_parcel_mailing_list(
        &self,
        parcel_id: &str,
        slug: &str,
        owner_player_id: &str,
        ...
        validate_slug(slug)?;
        let parcel = self.owned_built_parcel(parcel_id, owner_player_id).await?;
        self.parcel_mailing_list_by_parcel_slug(&parcel.parcel_id, slug)
            .await?
    ...
    async fn parcel_mailing_list_by_parcel_slug(
        &self,
        parcel_id: &str,
        slug: &str,
    ) -> Result<Option<StoredParcelMailingList>, StorageError> {
        let row = sqlx::query_as::<_, StoredParcelMailingList>(
            r#"
            select l.id, l.parcel_id, l.owner_player_id, l.slug, l.title, l.status,
                   coalesce(active_subscribers.count, 0)::bigint as subscriber_count,
    ...
    async fn parcel_mailing_list_count(&self, parcel_id: &str) -> Result<usize, StorageError> {
        let count = sqlx::query_scalar::<_, i64>(
            r#"
            select count(*)::bigint
            from parcel_mailing_lists
            where parcel_id = $1
            "#,
        )
        .bind(parcel_id)
    ...
    async fn resolve_parcel_mailing_list(
        &self,
        target: &str,
        slug: &str,
    ) -> Result<StoredParcelMailingList, StorageError> {
    ...
    async fn subscription_for_player(
        ...
        let row = sqlx::query_as::<_, StoredParcelMailingListSubscription>(
            r#"
            select l.parcel_id, p.title as parcel_title, l.slug, l.title as list_title,
                   s.status,
                   to_char(s.updated_at, 'YYYY-MM-DD HH24:MI:SS TZ') as updated_at
    ...
    async fn parcel_command_route_targets(
        &self,
        parcel_id: &str,
    ) -> Result<Vec<ParcelCommandRouteTarget>, StorageError> {
        let parcel_id = canonical_parcel_id(parcel_id);
        let targets = sqlx::query_as::<_, ParcelCommandRouteTarget>(
            r#"
    ...
    async fn insert_parcel_mailing_list_post_for_recipients(
        &self,
        insert: MailingListPostInsert<'_>,
    ) -> Result<StoredParcelMailingListPost, StorageError> {
        let recipient_count = i64::try_from(insert.recipients.len()).map_err(|_| {
            StorageError::InvalidMailingList("subscriber count exceeds supported range".to_owned())
        })?;
        let mut tx = self.pool.begin().await?;
        let post = sqlx::query_as::<_, StoredParcelMailingListPost>(
            r#"
    ...
    async fn parcel_mailing_list_post(
        &self,
        post_id: i64,
    ) -> Result<StoredParcelMailingListPost, StorageError> {
        let post = sqlx::query_as::<_, StoredParcelMailingListPost>(
            ...
            from parcel_mailing_list_posts post
            join parcel_mailing_lists list on list.id = post.list_id
            where post.id = $1
            "#,
        )
        .bind(post_id)
...
fn parcel_work_item_projection() -> &'static str {
    r#"
...
fn parcel_work_item_projection_with_subqueries() -> &'static str {
    r#"
    item.id, item.parcel_id,
    (select d.slug from parcel_work_desks d where d.id = item.desk_id) as slug,
    (select d.title from parcel_work_desks d where d.id = item.desk_id) as desk_title,
    item.operator_command_id, item.command_prefix, item.status,
...
fn matching_parcel_command_route_targets(
    targets: impl IntoIterator<Item = ParcelCommandRouteTarget>,
    raw_input: &str,
) -> Vec<ParcelCommandRouteTarget> {
    let mut target_indexes = HashMap::<i64, usize>::new();
    let mut matches = Vec::<ParcelCommandRouteTarget>::new();
    for target in targets {
...
fn sender_can_post_to_parcel_chat(
